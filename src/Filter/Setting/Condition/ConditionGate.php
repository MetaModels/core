...
use MetaModels\Filter\Rules\Condition\ConditionGate as FilterRuleGate;
 ...
 */
final class ConditionGate implements IWithChildren
{
    /** @var list<ISimple> */
    ...
    public function prepareRules(IFilter $objFilter, $arrFilterUrl): void
    {
        ...
        if ($this->children[1] ?? null) {
            ...
            $this->children[1]->prepareRules($ifFalse, $arrFilterUrl);
        }

        $filterRule = new FilterRuleGate(
            $this->getExpression(),
        ...
        if (count($this->children) >= 2) {
            ...
            Message::addInfo(
                $this->translator->trans(
                    ...
                    ['%name%' => 'conditiongate', '%max%' => '2'],
                    'tl_metamodel_filtersetting'
                )
            );
            return;
            //throw new RuntimeException('A condition gate can only have two children max.');
        }
    ...
    {
        return (string) $this->data['gate_expression'];
    }
