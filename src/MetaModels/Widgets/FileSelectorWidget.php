...
class FileSelectorWidget extends \FileSelector
{
    ...
    public function generate()
    {
        ...
        $arrTmp = explode('_', $this->strField);
        array_pop($arrTmp);
        $this->strDcaField = implode('_', $arrTmp);
        ...
        if ($strNode != '' && isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['path']))
        {
            if (strncmp($strNode . '/', $GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['path'] . '/', strlen($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['path']) + 1) !== 0)
            {
        ...
        elseif (isset($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['path']))
        {
            $tree = $this->renderFiletree(TL_ROOT . '/' . $GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['path'], 0);
        }
        ...
        if ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['fieldType'] == 'checkbox')
        {
    ...
    public function generateAjax($folder, $strField, $level, $mount=false)
    {
        ...
        switch ($GLOBALS['TL_DCA'][$this->strTable]['config']['dataContainer'])
        {
            case 'File':
                ...
                {
                    $this->varValue = $GLOBALS['TL_CONFIG'][$this->strDcaField];
                }
                break;
            ...
            case 'Table':
                ...
                if (!$this->Database->fieldExists($this->strDcaField, $this->strTable))
                {
                    break;
                }

                $objField = $this->Database->prepare("SELECT " . $this->strDcaField . " FROM " . $this->strTable . " WHERE id=?")
                                           ->limit(1)
                                           ->execute($this->strId);

                if ($objField->numRows)
                {
                    $this->varValue = deserialize($objField->{$this->strDcaField});
                }
                break;
        }
    ...
    protected function renderFiletree($path, $intMargin, $mount=false)
    {
        ...
        $folderClass = ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['files'] || $GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['filesOnly']) ? 'tl_folder' : 'tl_file';
        ...
        for ($f=0, $c=count($folders); $f<$c; $f++)
        {
            ...
            {
                if (is_dir($folders[$f].'/'.$v) || $GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['files'] || $GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['filesOnly'])
                {
                    $countFiles++;
                }
            }
            ...
            $folderLabel = ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['files'] || $GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['filesOnly']) ? '<strong>'.specialchars(basename($currentFolder)).'</strong>' : specialchars(basename($currentFolder));
            ...
            if (!$GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['filesOnly'])
            {
                switch ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['fieldType'])
                {
                    case 'checkbox':
        ...
        // Process files
        if ($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['files'] || $GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['filesOnly'])
        {
            $allowedExtensions = null;

            if (strlen($GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['extensions']))
            {
                $allowedExtensions = trimsplit(',', $GLOBALS['TL_DCA'][$this->strTable]['fields'][$this->strDcaField]['eval']['extensions']);
            }
