...
/**
 * Manipulate the data definition for the property "rendersortattr" in table "tl_metamodel_dca_sortgroup".
 *
 * @package MetaModels\DcGeneral\Events\Table\InputScreen
 */
class PropertyRenderSortAttribute
{
    /**
    ...
    public static function getMetaModel(EnvironmentInterface $interface)
    {
        $metaModelId = \Database::getInstance()
            ->prepare(
                'SELECT id FROM tl_metamodel WHERE
                id=(SELECT pid FROM tl_metamodel_dca WHERE id=?)'
            )
