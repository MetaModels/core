...
/**
 * This class is used from DCA tl_metamodel_dca_combine for various callbacks.
 *
 * @package    MetaModels
 * @subpackage Backend
 * @author     Christian Schiffler <c.schiffler@cyberspectrum.de>
...
class DcaCombine
{
	protected function getGroups($strTable='user')
	{
		...
		{
			throw new \RuntimeException('unkown table name ' . $strTable, 1);
		}
		$objGroups = \Database::getInstance()->execute(sprintf('SELECT id,name FROM tl_%s_group', $strTable));
	...
	{
		$objPalettes = \Database::getInstance()->prepare('SELECT id,name FROM tl_metamodel_dca WHERE pid=?')->execute($intMetaModel);
	...
	{
		$objViews = \Database::getInstance()->prepare('SELECT id,name FROM tl_metamodel_rendersettings WHERE pid=?')->execute($intMetaModel);
	...
	{
		return $this->getPalettes(\Input::getInstance()->get('id')/*$objDC->getId()*/);
	}
	...
	{
		return $this->getViews(\Input::getInstance()->get('id')/*$objDC->getId()*/);
	}
